/**
 * better-auth's tables — `docs/architecture/design/schema.md` §5.1.
 *
 * Not designed here. The starting point is
 *
 ...
 *    `getAuthTables()` at the installed version declares it required, and
 *    sign-up writes it, so a schema without it fails on the first sign-in.
 *  - `lib/auth/auth.ts` exports a `getAuth()` factory rather than a bare `auth`,
 *    which the CLI cannot read, so the config was passed to it as a temporary
