 ...
 *
 * One relation restricted by another's `user_id` is safe only while that other
 * one is restricted, so at least one relation per plan has to be restricted to
 * the value `requireUser()` produced. A join between two table scans cannot close that
 * chain on itself — a plan is a tree, and its driving scan has no outer
 * relation to reference — but a plan whose leaves are not all table scans can:
 * a `CTE`, `VALUES` or function relation supplying the `user_id` renders as
 * `(user_id = "*VALUES*".column1)`, which is bound and vouched for by nothing.
 *
...
export function relationScansOf(plan: PlanNode[]): RelationScan[] {
  ...
  const walk = (node: PlanNode, owner: RelationScan | undefined) => {
    let next = owner;
    ...
    for (const child of node.Plans ?? []) walk(child, next);
  };
 ...
 *     of a relation the plan has already restricted;
 ...
 *
 * Resolved as a fixpoint, because (2) depends on the parent being restricted
 * already and the parent may appear anywhere in the plan.
 */
export function unboundRelations(
  ...
  const tableOf = new Map(scans.map((scan) => [scan.alias, scan.table]));
  const bound = new Set(
    scans
      .filter((scan) => BINDS_USER_ID.test(restrictionOf(scan)))
      .map((scan) => scan.alias),
  );
  ...
  const boundThroughAKey = (scan: RelationScan): boolean =>
    foreignKeys
      ...
      .some((fk) => {
        ...
        return tableOf.get(alias) === fk.parent && bound.has(alias);
      });

  for (let settled = false; !settled; ) {
    settled = true;
    for (const scan of scans) {
      if (bound.has(scan.alias)) continue;
      if (boundThroughAKey(scan)) {
        bound.add(scan.alias);
        settled = false;
      }
    }
  }

  return scans.filter((scan) => !bound.has(scan.alias));
}
