...
Before pushing, run **`npm run gate`**. It resolves the changed files, selects the checks those files need, and runs all of them without stopping at the first failure. Its check list is held in step with `.github/workflows/ci.yml` by `scripts/gate/checks.test.ts` (ADR-011), so do not assemble the commands by hand and do not rely on a list of them kept anywhere else — the table and its routing are stated once in `scripts/gate/checks.ts` and described in `docs/architecture/design/T-026-gate-and-ledger.md` §2. A check it cannot run here is reported as skipped with the reason, never as passed. Each run appends a row per check to the gitignored `.gate/ledger.jsonl`.
