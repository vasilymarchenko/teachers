...
The scaffold (T-002), the database schema (T-004), the schedule domain (T-005), the auth boundary (T-006), the calendar read queries (T-008), the calendar read views (T-007), the application shell (T-014), the deploy pipeline (T-015), the year-setup screens (T-009), the weekly template editor (T-010) and the CI gate (T-024) are in place: the app builds and runs against a migrated Postgres, behind sign-in, with the navigation panel, a working calendar — day, week, month and year over real schedule data — a `/year` screen the teacher enters that data on and a `/schedule` screen where the weekly template is edited under copy-on-write versioning, while the event screen of the first release is still a placeholder; it is deployable to a VPS via Docker Compose, GHCR and Caddy, and every pushed commit is checked by `.github/workflows/ci.yml` — the five commands below plus both Docker images — with the GHCR publish gated on that run (ADR-007). The first writes landed with T-009: `lib/db/queries` stays read-only and every mutation is a Server Action in `lib/actions` going to Drizzle directly, which is what `architect-overview.md` §2 prescribes for CRUD forms. Day overrides and events are still read-only. The planning documents are:
