...
Before pushing, run `npm run gate`. It selects the checks the change actually needs — always `lint`, `typecheck`, `test` and `build`, plus the database checks for `lib/db/**` or `drizzle/**` and the image builds for the `Dockerfile` or a Compose file — runs all of them without stopping at the first failure, prints one table and exits non-zero if any failed. A check it cannot run here (no Docker daemon, no `DATABASE_URL`) is reported `skipped` with the reason, which is not a pass. The routing lives in `scripts/gate/checks.ts` and is held in step with `.github/workflows/ci.yml` by a convention test — `docs/architecture/decisions/ADR-012-one-check-definition.md`. A single test file: `npx vitest run lib/time/today.test.ts`.
